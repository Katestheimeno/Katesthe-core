...
# NOTE: the codes `image_too_large` / `image_invalid_type` are intended to map to
# `VALIDATION__*` envelope codes via utils/drf_error_envelope.py::_FIELD_CODE_MAP if a
# project wants coded envelope errors. Do NOT edit that file here (owned by subtask 003);
# a future task can extend the map.
...
def validate_image_size(file, max_bytes: int = MAX_IMAGE_UPLOAD_BYTES):
    """Reject image uploads larger than `max_bytes`. Silent no-op if file/size absent."""
...
def validate_image_mime(file, allowed=ALLOWED_IMAGE_MIME):
    """Reject image uploads whose content_type is not in `allowed`. Silent no-op if file/content_type absent."""
